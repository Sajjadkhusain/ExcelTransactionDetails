  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (evt) => {
      ...
      const headersIndex = jsonData.findIndex(
        (row) => row.includes("S.No") && row.includes("Ration Card No")
      );

      const rows = jsonData
        .slice(headersIndex + 1)
        .filter((row) => row[0] || row[1] || row[2]); // allow rows with any data in columns A–C
          ...
          <table className="table table-bordered table-striped mb-0">
            <thead className="table-light">
              <tr>
                ...
                <th className="headerColor">Family Member</th>
              </tr>
            </thead>
            <tbody>
